...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--name_of_subject', type=str, help='')
    parser.add_argument(
        '--save_fps', type=int, default=24, help='Frame per second. Default: 24')
    parser.add_argument(
        '--save_frames_behave', action='store_true', help='Save output frames in behave way. Default: False')
    parser.add_argument(
    ...
    if args.save_frames:
     ...
     # save each frame in the behave format
    if args.name_of_subject is not None:
        name_of_subject = args.name_of_subject
    if args.save_frames_behave:
        for idx in range(video_length):
            f = comp_frames[idx]
            f = cv2.resize(f, out_size, interpolation = cv2.INTER_CUBIC)
            f = cv2.cvtColor(f, cv2.COLOR_BGR2RGB)
            img_save_root = os.path.join(save_root, 'images', name_of_subject+str(idx).zfill(4)+'.jpg')
            imwrite(f, img_save_root)                   
